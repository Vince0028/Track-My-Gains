 ...
 * @param corrections - User's corrections like "2x Adobo, 1x Rice" to help AI re-analyze
 */
export async function analyzeFood(base64Image, mode = 'food', weightHint = null, userProfile = null, corrections = null) {
    let lastError = null;

    // 1. Try Gemini (Strongest)
    try {
        console.log("Scanner: Attempting Gemini Vision...");
        const result = await analyzeImageWithGemini(base64Image, mode, weightHint, userProfile, corrections);
        return result;
    } catch (error) {
    ...
    try {
        ...
        const result = await analyzeImageWithGroq(base64Image, mode, weightHint, userProfile, corrections);
        return result;
    } catch (error) {
